...
import type { SidakGerindaDudukSession, SidakGerindaDudukRecord, SidakGerindaDudukObserver } from "@shared/schema";
import { format } from "date-fns";
import { id } from "date-fns/locale";
import { useState, useRef } from "react";
import { apiRequest } from "@/lib/queryClient";

interface SessionWithDetails extends SidakGerindaDudukSession {
    records?: SidakGerindaDudukRecord[];
    observers?: SidakGerindaDudukObserver[];
}

export default function SidakGerindaDudukHistory() {
    ...
    const { data: sessions, isLoading } = useQuery<SessionWithDetails[]>({
        queryKey: ['/api/sidak-gerinda-duduk/sessions'],
    });

    const uploadPhotosMutation = useMutation({
        mutationFn: async ({ sessionId, base64Photos }: { sessionId: string; base64Photos: string[] }) => {
            const res = await apiRequest(`/api/sidak-gerinda-duduk/${sessionId}/photos`, "POST", {
                photos: base64Photos
            });
            return res;
        },
    ...
    const deletePhotoMutation = useMutation({
        mutationFn: async ({ sessionId, photoIndex }: { sessionId: string; photoIndex: number }) => {
            const res = await apiRequest(`/api/sidak-gerinda-duduk/${sessionId}/photos/${photoIndex}`, "DELETE");
            return res;
        },
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['/api/sidak-gerinda-duduk/sessions'] });
    ...
    const handleDownloadPDF = async (sessionId: string) => {
        try {
            ...
            const response = await fetch(`/api/sidak-gerinda-duduk/${sessionId}`);
            if (!response.ok) throw new Error('Gagal mengambil data session');
            const data = await response.json();

            const { generateSidakGerindaDudukPDF } = await import('@/lib/sidak-gerinda-duduk-pdf-utils');
            const doc = await generateSidakGerindaDudukPDF({
                session: data.session,
                records: data.records,
                observers: data.observers
            });

            const fileName = `Sidak_Gerinda_Duduk_${data.session.tanggal}_${data.session.id.substring(0, 5)}.pdf`;
            doc.save(fileName);
    ...
    const handleDownloadJPG = async (sessionId: string) => {
        try {
            ...
            const { downloadSidakGerindaDudukAsJpg } = await import('@/lib/sidak-gerinda-duduk-pdf-utils');
            const fileName = `Sidak_Gerinda_Duduk_${data.session.tanggal}_${Date.now()}.jpg`;
            await downloadSidakGerindaDudukAsJpg({
                session: data.session,
                records: data.records,
                observers: data.observers
            }, fileName);
                ...
                {isLoading ? (
                    ...
                    <Card className="text-center py-12 border-dashed border-2 border-gray-200">
                        <CardContent>
                            ...
                            <p className="text-gray-500 font-medium">Belum ada riwayat inspeksi gerinda duduk</p>
                            <EmptyStateCreateButton href="/workspace/sidak/gerinda-duduk/new" label="Mulai Inspeksi Baru" className="mt-6 bg-blue-600 hover:bg-blue-700 text-white" />
                        </CardContent>
                    </Card>
                ) : (
