...
import type { SidakBottleJackSession, SidakBottleJackRecord, SidakBottleJackObserver } from "@shared/schema";
import { format } from "date-fns";
import { id } from "date-fns/locale";
import { useState, useRef } from "react";
import { apiRequest } from "@/lib/queryClient";

interface SessionWithDetails extends SidakBottleJackSession {
    records?: SidakBottleJackRecord[];
    observers?: SidakBottleJackObserver[];
}

export default function SidakBottleJackHistory() {
    ...
    const { data: sessions, isLoading } = useQuery<SessionWithDetails[]>({
        queryKey: ['/api/sidak-bottle-jack/sessions'],
    });

    const uploadPhotosMutation = useMutation({
        mutationFn: async ({ sessionId, base64Photos }: { sessionId: string; base64Photos: string[] }) => {
            const res = await apiRequest(`/api/sidak-bottle-jack/${sessionId}/photos`, "POST", {
                photos: base64Photos
            });
            return res;
        },
    ...
    const deletePhotoMutation = useMutation({
        mutationFn: async ({ sessionId, photoIndex }: { sessionId: string; photoIndex: number }) => {
            const res = await apiRequest(`/api/sidak-bottle-jack/${sessionId}/photos/${photoIndex}`, "DELETE");
            return res;
        },
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['/api/sidak-bottle-jack/sessions'] });
    ...
    const handleDownloadPDF = async (sessionId: string) => {
        try {
            ...
            const response = await fetch(`/api/sidak-bottle-jack/${sessionId}`);
            if (!response.ok) throw new Error('Gagal mengambil data session');
            const data = await response.json();

            const { generateSidakBottleJackPDF } = await import('@/lib/sidak-bottle-jack-pdf-utils');
            const doc = await generateSidakBottleJackPDF({
                session: data.session,
                records: data.records,
                observers: data.observers
            });

            const fileName = `Sidak_Bottle_Jack_${data.session.tanggal}_${data.session.id.substring(0, 5)}.pdf`;
            doc.save(fileName);
    ...
    const handleDownloadJPG = async (sessionId: string) => {
        try {
            ...
            const { downloadSidakBottleJackAsJpg } = await import('@/lib/sidak-bottle-jack-pdf-utils');
            const fileName = `Sidak_Bottle_Jack_${data.session.tanggal}_${Date.now()}.jpg`;
            await downloadSidakBottleJackAsJpg({
                session: data.session,
                records: data.records,
                observers: data.observers
            }, fileName);
                ...
                {isLoading ? (
                    ...
                    <Card className="text-center py-12 border-dashed border-2 border-gray-200">
                        <CardContent>
                            ...
                            <p className="text-gray-500 font-medium">Belum ada riwayat inspeksi bottle jack</p>
                            <EmptyStateCreateButton href="/workspace/sidak/bottle-jack/new" label="Mulai Inspeksi Baru" className="mt-6 bg-orange-600 hover:bg-orange-700" />
                        </CardContent>
                    </Card>
                ) : (
                            ...
                            <Card key={session.id} className="overflow-hidden border-none shadow-sm rounded-2xl bg-white dark:bg-gray-800 ring-1 ring-gray-100">
                                ...
                                <CardContent className="p-5">
                                    <div className="flex justify-between items-start mb-4">
                                        ...
                                        <Badge className="bg-orange-50 text-orange-700 border-none rounded-full px-3 py-1">
                                            {session.totalBottleJack} Unit
                                        </Badge>
                                    </div>
                                        ...
                                        <div className="mb-4">
                                            <div className="flex gap-2">
                                                ...
                                                {session.activityPhotos.length > 3 && (
                                                    <div
                                                        className="aspect-square w-14 h-14 rounded-xl bg-orange-50 text-orange-600 flex items-center justify-center cursor-pointer font-bold text-xs"
                                                        onClick={() => handleOpenPhotoDialog(session)}
                                                    >
                                                        +{session.activityPhotos.length - 3}
                                                    </div>
