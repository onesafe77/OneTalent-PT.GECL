...
import { Download, Calendar, Clock, ArrowLeft, ChevronDown, Camera, FileText, Image, Shield, Trash2 } from "lucide-react";
...
interface SessionWithDetails {
    ...
    createdBy?: string;
    record?: { inspectionResults?: Record<string, string> } | null;
    records?: Array<{ inspectionResults?: Record<string, string> }>;
    observers?: Array<{ nama: string; perusahaan?: string }>;
}

function countTS(session: SessionWithDetails): number {
    const record = session.record || session.records?.[0];
    if (!record?.inspectionResults) return 0;
    return Object.values(record.inspectionResults).filter(v => v === "TS").length;
}

export default function SidakPemenuhanTyreHistory() {
    ...
    const { data: sessions, isLoading } = useQuery<SessionWithDetails[]>({
        queryKey: ['/api/sidak-pemenuhan-tyre/sessions'],
    });

    const uploadPhotosMutation = useMutation({
        mutationFn: async ({ sessionId, base64Photos }: { sessionId: string; base64Photos: string[] }) => {
            const res = await apiRequest(`/api/sidak-pemenuhan-tyre/${sessionId}/photos`, "POST", {
                photos: base64Photos
            });
            return res;
        },
    ...
    const deletePhotoMutation = useMutation({
        mutationFn: async ({ sessionId, photoIndex }: { sessionId: string; photoIndex: number }) => {
            const res = await apiRequest(`/api/sidak-pemenuhan-tyre/${sessionId}/photos/${photoIndex}`, "DELETE");
            return res;
        },
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['/api/sidak-pemenuhan-tyre/sessions'] });
    ...
    const deleteSessionMutation = useMutation({
        mutationFn: async (sessionId: string) => {
            await apiRequest(`/api/sidak-pemenuhan-tyre/${sessionId}`, "DELETE");
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['/api/sidak-pemenuhan-tyre/sessions'] });
    ...
    const handleDownloadPDF = async (sessionId: string) => {
        try {
            ...
            const response = await fetch(`/api/sidak-pemenuhan-tyre/${sessionId}`);
            if (!response.ok) throw new Error('Gagal mengambil data session');
            const data = await response.json();

            const { generateSidakPemenuhanTyrePdf } = await import('@/lib/sidak-pemenuhan-tyre-pdf-utils');
            const doc = await generateSidakPemenuhanTyrePdf({
                session: data.session,
                record: data.record,
                observers: data.observers
            });

            const fileName = `Sidak_PemenuhanTyre_${data.session.tanggal}.pdf`;
            doc.save(fileName);
    ...
    const handleDownloadJPG = async (sessionId: string) => {
        try {
            ...
            const { downloadSidakPemenuhanTyreAsJpg } = await import('@/lib/sidak-pemenuhan-tyre-pdf-utils');
            const fileName = `Sidak_PemenuhanTyre_${data.session.tanggal}_${Date.now()}.jpg`;
            await downloadSidakPemenuhanTyreAsJpg({
                session: data.session,
                record: data.record,
                observers: data.observers
            }, fileName);
                    ...
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                            ...
                            <h1 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white truncate">
                                Riwayat Sidak Pemenuhan Tyre
                            </h1>
                        </div>
                        <p className="text-[11px] text-gray-500 truncate">
                            Form BIB-CLR-SKR-F-014-01 - Checklist Inspeksi Pemenuhan Standar Tyre Management
                        </p>
                    </div>
                </div>
                ...
                {isLoading ? (
                    ...
                    <Card className="text-center py-12 border-dashed border-2 border-gray-200">
                        <CardContent>
                            ...
                            <p className="text-gray-500 font-medium">Belum ada riwayat sidak pemenuhan tyre</p>
                            <EmptyStateCreateButton href="/workspace/sidak/pemenuhan-tyre/new" label="Mulai Inspeksi Baru" className="mt-6 bg-blue-600 hover:bg-blue-700 text-white" />
                        </CardContent>
                    </Card>
                ) : (
                    <div className="space-y-4">
                        {sessions.map((session) => {
                            const tsCount = countTS(session);
                            const observerNames = (session.observers || []).map(o => o.nama).filter(Boolean).join(', ');
                            return (
                                <Card key={session.id} className="overflow-hidden border-none shadow-sm rounded-2xl bg-white dark:bg-gray-800 ring-1 ring-gray-100 dark:ring-gray-700">
                                    <div className="h-1.5 w-full bg-blue-500" />
                                    <CardContent className="p-5">
                                        <div className="flex justify-between items-start mb-4">
                                            <div>
                                                <h3 className="text-lg font-bold text-gray-900 dark:text-white line-clamp-1">
                                                    {session.lokasi} - {session.shift || '-'}
                                                </h3>
                                            ...
                                            {tsCount > 0 ? (
                                                <Badge className="bg-red-100 text-red-700 hover:bg-red-100">{tsCount} TS</Badge>
                                            ) : (
                                                <Badge className="bg-blue-100 text-blue-700 hover:bg-blue-100">{PEMENUHAN_TYRE_ITEMS.length} Item</Badge>
                                            )}
                                        </div>

                                        <div className="bg-gray-50 dark:bg-gray-900/30 rounded-xl p-3 mb-4 space-y-1.5">
                                            <div className="flex items-start gap-2 text-xs">
                                                <span className="font-bold text-gray-400 uppercase w-24 flex-shrink-0">Lokasi</span>
                                                <span className="text-gray-700 dark:text-gray-300">{session.lokasi}</span>
                                            </div>
                                            <div className="flex items-start gap-2 text-xs">
                                                <span className="font-bold text-gray-400 uppercase w-24 flex-shrink-0">Perusahaan</span>
                                                <span className="text-gray-700 dark:text-gray-300">{session.namaPerusahaan || '-'}</span>
                                            </div>
                                            <div className="flex items-start gap-2 text-xs">
                                                <span className="font-bold text-gray-400 uppercase w-24 flex-shrink-0">Observer</span>
                                                <span className="text-gray-700 dark:text-gray-300">{observerNames || '-'}</span>
                                            </div>
                                        </div>
                                            ...
                                            <div className="mb-4">
                                                <div className="flex gap-2">
                                                    ...
                                                    {session.activityPhotos.length > 3 && (
                                                        <div
                                                            className="aspect-square w-14 h-14 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center cursor-pointer font-bold text-xs"
                                                            onClick={() => handleOpenPhotoDialog(session)}
                                                        >
                                                            +{session.activityPhotos.length - 3}
                                                        </div>
                                        ...
                                        <DropdownMenu>
                                            <DropdownMenuTrigger asChild>
                                                <Button className="w-full bg-blue-600 hover:bg-blue-700 text-white rounded-xl h-11 shadow-sm">
                                                    <Download className="h-4 w-4 mr-2" />
                                                    Pilihan Aksi
                                                    <ChevronDown className="h-4 w-4 ml-auto opacity-70" />
                                                </Button>
                                            </DropdownMenuTrigger>
                                            <DropdownMenuContent align="end" className="w-[calc(100vw-3rem)] sm:w-64 rounded-2xl p-2 shadow-xl border-none">
                                                ...
                                                <DropdownMenuItem
                                                    ...
                                                    className="rounded-xl py-3 cursor-pointer border-t mt-1 pt-3 focus:bg-red-50"
                                                >
                                                    <div className="h-8 w-8 rounded-full bg-red-50 text-red-600 flex items-center justify-center mr-3">
                                                        <Trash2 className="h-4 w-4" />
                                                    </div>
                                                    <div className="flex flex-col">
                                                        <span className="font-bold text-sm text-red-600">Hapus Sesi</span>
                                                        <span className="text-[10px] text-gray-500 uppercase tracking-wider font-bold">Tindakan Permanen</span>
                                                    </div>
                                                </DropdownMenuItem>
                                            </DropdownMenuContent>
                                        </DropdownMenu>
