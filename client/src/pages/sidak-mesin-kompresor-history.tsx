...
import type { SidakMesinKompresorSession, SidakMesinKompresorRecord, SidakMesinKompresorObserver } from "@shared/schema";
import { format } from "date-fns";
import { id } from "date-fns/locale";
import { useState, useRef } from "react";
import { apiRequest } from "@/lib/queryClient";

interface SessionWithDetails extends SidakMesinKompresorSession {
    records?: SidakMesinKompresorRecord[];
    observers?: SidakMesinKompresorObserver[];
}

export default function SidakMesinKompresorHistory() {
    ...
    const { data: sessions, isLoading } = useQuery<SessionWithDetails[]>({
        queryKey: ['/api/sidak-mesin-kompresor/sessions'],
    });

    const uploadPhotosMutation = useMutation({
        mutationFn: async ({ sessionId, base64Photos }: { sessionId: string; base64Photos: string[] }) => {
            const res = await apiRequest(`/api/sidak-mesin-kompresor/${sessionId}/photos`, "POST", {
                photos: base64Photos
            });
            return res;
        },
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['/api/sidak-mesin-kompresor/sessions'] });
    ...
    const deletePhotoMutation = useMutation({
        mutationFn: async ({ sessionId, photoIndex }: { sessionId: string; photoIndex: number }) => {
            const res = await apiRequest(`/api/sidak-mesin-kompresor/${sessionId}/photos/${photoIndex}`, "DELETE");
            return res;
        },
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['/api/sidak-mesin-kompresor/sessions'] });
    ...
    const handleDownloadPDF = async (sessionId: string) => {
        try {
            ...
            const response = await fetch(`/api/sidak-mesin-kompresor/${sessionId}`);
            if (!response.ok) throw new Error('Gagal mengambil data session');
            const data = await response.json();

            const { generateSidakMesinKompresorPDF } = await import('@/lib/sidak-mesin-kompresor-pdf-utils');
            const doc = await generateSidakMesinKompresorPDF({
                session: data.session,
                records: data.records,
                observers: data.observers
            });

            const fileName = `Sidak_Mesin_Kompresor_${data.session.tanggal}_${data.session.id.substring(0, 5)}.pdf`;
            doc.save(fileName);
    ...
    const handleDownloadJPG = async (sessionId: string) => {
        try {
            ...
            const { downloadSidakMesinKompresorAsJpg } = await import('@/lib/sidak-mesin-kompresor-pdf-utils');
            const fileName = `Sidak_Mesin_Kompresor_${data.session.tanggal}_${Date.now()}.jpg`;
            await downloadSidakMesinKompresorAsJpg({
                session: data.session,
                records: data.records,
                observers: data.observers
            }, fileName);
                ...
                {isLoading ? (
                    ...
                    <Card className="text-center py-12 border-dashed border-2 border-gray-200">
                        <CardContent>
                            ...
                            <p className="text-gray-500 font-medium">Belum ada riwayat inspeksi mesin kompresor</p>
                            <EmptyStateCreateButton href="/workspace/sidak/mesin-kompresor/new" label="Mulai Inspeksi Baru" className="mt-6 bg-blue-600 hover:bg-blue-700 text-white" />
                        </CardContent>
                    </Card>
                ) : (
                            ...
                            <Card key={session.id} className="overflow-hidden border-none shadow-sm rounded-2xl bg-white dark:bg-gray-800 ring-1 ring-gray-100 dark:ring-gray-700">
                                ...
                                <CardContent className="p-5">
                                    <div className="flex justify-between items-start mb-4">
                                        <div>
                                            <h3 className="text-lg font-bold text-gray-900 dark:text-white line-clamp-1">{session.namaObjekInspeksi}</h3>
                                            <p className="text-xs text-blue-600 font-semibold">{session.lokasi}</p>
