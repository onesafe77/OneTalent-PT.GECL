...
import type { SidakStandJackSession, SidakStandJackRecord, SidakStandJackObserver } from "@shared/schema";
import { format } from "date-fns";
import { id } from "date-fns/locale";
import { useState, useRef } from "react";
import { apiRequest } from "@/lib/queryClient";

interface SessionWithDetails extends SidakStandJackSession {
    records?: SidakStandJackRecord[];
    observers?: SidakStandJackObserver[];
}

export default function SidakStandJackHistory() {
    ...
    const { data: sessions, isLoading } = useQuery<SessionWithDetails[]>({
        queryKey: ['/api/sidak-stand-jack/sessions'],
    });

    const uploadPhotosMutation = useMutation({
        mutationFn: async ({ sessionId, base64Photos }: { sessionId: string; base64Photos: string[] }) => {
            const res = await apiRequest(`/api/sidak-stand-jack/${sessionId}/photos`, "POST", {
                photos: base64Photos
            });
            return res;
        },
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['/api/sidak-stand-jack/sessions'] });
    ...
    const deletePhotoMutation = useMutation({
        mutationFn: async ({ sessionId, photoIndex }: { sessionId: string; photoIndex: number }) => {
            const res = await apiRequest(`/api/sidak-stand-jack/${sessionId}/photos/${photoIndex}`, "DELETE");
            return res;
        },
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['/api/sidak-stand-jack/sessions'] });
    ...
    const handleDownloadPDF = async (sessionId: string) => {
        try {
            ...
            const response = await fetch(`/api/sidak-stand-jack/${sessionId}`);
            if (!response.ok) throw new Error('Gagal mengambil data session');
            const data = await response.json();

            const { generateSidakStandJackPDF } = await import('@/lib/sidak-stand-jack-pdf-utils');
            const doc = await generateSidakStandJackPDF({
                session: data.session,
                records: data.records,
                observers: data.observers
            });

            const fileName = `Sidak_Stand_Jack_${data.session.tanggal}_${data.session.id.substring(0, 5)}.pdf`;
            doc.save(fileName);
    ...
    const handleDownloadJPG = async (sessionId: string) => {
        try {
            ...
            const { downloadSidakStandJackAsJpg } = await import('@/lib/sidak-stand-jack-pdf-utils');
            const fileName = `Sidak_Stand_Jack_${data.session.tanggal}_${Date.now()}.jpg`;
            await downloadSidakStandJackAsJpg({
                session: data.session,
                records: data.records,
                observers: data.observers
            }, fileName);
                ...
                {isLoading ? (
                    ...
                    <Card className="text-center py-12 border-dashed border-2 border-gray-200">
                        <CardContent>
                            ...
                            <EmptyStateCreateButton href="/workspace/sidak/stand-jack/new" label="Mulai Inspeksi Baru" className="mt-6 bg-orange-600 hover:bg-orange-700" />
                        </CardContent>
                    </Card>
                ) : (
