...
import type { SidakHydraulicJackSession, SidakHydraulicJackRecord, SidakHydraulicJackObserver } from "@shared/schema";
import { format } from "date-fns";
import { id } from "date-fns/locale";
import { useState, useRef } from "react";
import { apiRequest } from "@/lib/queryClient";

interface SessionWithDetails extends SidakHydraulicJackSession {
    records?: SidakHydraulicJackRecord[];
    observers?: SidakHydraulicJackObserver[];
}

export default function SidakHydraulicJackHistory() {
    ...
    const { data: sessions, isLoading } = useQuery<SessionWithDetails[]>({
        queryKey: ['/api/sidak-hydraulic-jack/sessions'],
    });

    const uploadPhotosMutation = useMutation({
        mutationFn: async ({ sessionId, base64Photos }: { sessionId: string; base64Photos: string[] }) => {
            const res = await apiRequest(`/api/sidak-hydraulic-jack/${sessionId}/photos`, "POST", {
                photos: base64Photos
            });
            return res;
        },
    ...
    const deletePhotoMutation = useMutation({
        mutationFn: async ({ sessionId, photoIndex }: { sessionId: string; photoIndex: number }) => {
            const res = await apiRequest(`/api/sidak-hydraulic-jack/${sessionId}/photos/${photoIndex}`, "DELETE");
            return res;
        },
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['/api/sidak-hydraulic-jack/sessions'] });
    ...
    const handleDownloadPDF = async (sessionId: string) => {
        try {
            ...
            const response = await fetch(`/api/sidak-hydraulic-jack/${sessionId}`);
            if (!response.ok) throw new Error('Gagal mengambil data session');
            const data = await response.json();

            const { generateSidakHydraulicJackPDF } = await import('@/lib/sidak-hydraulic-jack-pdf-utils');
            const doc = await generateSidakHydraulicJackPDF({
                session: data.session,
                records: data.records,
                observers: data.observers
            });

            const fileName = `Sidak_Hydraulic_Jack_${data.session.tanggal}_${data.session.id.substring(0, 5)}.pdf`;
            doc.save(fileName);
    ...
    const handleDownloadJPG = async (sessionId: string) => {
        try {
            ...
            const { downloadSidakHydraulicJackAsJpg } = await import('@/lib/sidak-hydraulic-jack-pdf-utils');
            const fileName = `Sidak_Hydraulic_Jack_${data.session.tanggal}_${Date.now()}.jpg`;
            await downloadSidakHydraulicJackAsJpg({
                session: data.session,
                records: data.records,
                observers: data.observers
            }, fileName);
                                ...
                                <CardContent className="p-5">
                                    <div className="flex justify-between items-start mb-4">
                                        ...
                                        <Badge className="bg-orange-50 text-orange-700 border-none rounded-full px-3 py-1">
                                            {session.totalHydraulicJack} Unit
                                        </Badge>
                                    </div>
