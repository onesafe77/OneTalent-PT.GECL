...
import type { SidakFuelStorageSession, SidakFuelStorageRecord, SidakFuelStorageObserver } from "@shared/schema";
import { format } from "date-fns";
import { id } from "date-fns/locale";
import { useState, useRef } from "react";
import { apiRequest } from "@/lib/queryClient";

interface SessionWithDetails extends SidakFuelStorageSession {
    records?: SidakFuelStorageRecord[];
    observers?: SidakFuelStorageObserver[];
}

export default function SidakFuelStorageHistory() {
    ...
    const { data: sessions, isLoading } = useQuery<SessionWithDetails[]>({
        queryKey: ['/api/sidak-fuel-storage/sessions'],
    });

    const uploadPhotosMutation = useMutation({
        mutationFn: async ({ sessionId, base64Photos }: { sessionId: string; base64Photos: string[] }) => {
            const res = await apiRequest(`/api/sidak-fuel-storage/${sessionId}/photos`, "POST", {
                photos: base64Photos
            });
            return res;
        },
    ...
    const deletePhotoMutation = useMutation({
        mutationFn: async ({ sessionId, photoIndex }: { sessionId: string; photoIndex: number }) => {
            const res = await apiRequest(`/api/sidak-fuel-storage/${sessionId}/photos/${photoIndex}`, "DELETE");
            return res;
        },
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['/api/sidak-fuel-storage/sessions'] });
    ...
    const handleDownloadPDF = async (sessionId: string) => {
        try {
            ...
            const response = await fetch(`/api/sidak-fuel-storage/${sessionId}`);
            if (!response.ok) throw new Error('Gagal mengambil data session');
            const data = await response.json();

            const { generateSidakFuelStoragePDF } = await import('@/lib/sidak-fuel-storage-pdf-utils');
            const doc = await generateSidakFuelStoragePDF({
                session: data.session,
                records: data.records,
                observers: data.observers
            });

            const fileName = `Sidak_Fuel_Storage_${data.session.tanggal}_${data.session.id.substring(0, 5)}.pdf`;
            doc.save(fileName);
    ...
    const handleDownloadJPG = async (sessionId: string) => {
        try {
            ...
            const { downloadSidakFuelStorageAsJpg } = await import('@/lib/sidak-fuel-storage-pdf-utils');
            const fileName = `Sidak_Fuel_Storage_${data.session.tanggal}_${Date.now()}.jpg`;
            await downloadSidakFuelStorageAsJpg({
                session: data.session,
                records: data.records,
                observers: data.observers
            }, fileName);
                ...
                {isLoading ? (
                    ...
                    <Card className="text-center py-12 border-dashed border-2 border-gray-200">
                        <CardContent>
                            ...
                            <p className="text-gray-500 font-medium">Belum ada riwayat inspeksi fuel storage</p>
                            <EmptyStateCreateButton href="/workspace/sidak/fuel-storage/new" label="Mulai Inspeksi Baru" className="mt-6 bg-blue-600 hover:bg-blue-700" />
                        </CardContent>
                    </Card>
                ) : (
                                ...
                                <CardContent className="p-5">
                                    <div className="flex justify-between items-start mb-4">
                                        <div>
                                            ...
                                            <p className="text-xs text-blue-600 font-semibold">{session.subLokasi}</p>
