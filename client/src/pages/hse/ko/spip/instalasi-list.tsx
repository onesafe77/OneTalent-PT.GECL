...
export default function SPIPInstalasi() {
    ...
    const { data: instalasiData, isLoading } = useQuery<any>({
        queryKey: ["/api/spip/instalasi", { search, kategori: kategoriFilter, area_lokasi: areaFilter, status_sertifikat: statusFilter, page }],
        queryFn: async () => {
            ...
            const res = await fetch(`/api/spip/instalasi?${params.toString()}`);
    ...
    const handleDelete = async (id: string) => {
        try {
            await apiRequest(`/api/spip/instalasi/${id}`, "DELETE");
            queryClient.invalidateQueries({ queryKey: ["/api/spip/instalasi"] });
            toast({ title: "Berhasil", description: "Data instalasi berhasil dihapus" });
        } catch (error: any) {
    ...
    const handleExport = async () => {
        window.open("/api/spip/instalasi/export", "_blank");
    };

    const items = instalasiData?.data || [];
    const total = instalasiData?.total || 0;
    ...
    const kategoriCount = new Set(items.map((item: any) => item.kategori)).size;
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                            <div className="p-4 w-64 space-y-4">
                                <p className="text-xs text-muted-foreground">Pilih file excel untuk mengimpor data instalasi.</p>
                                <Input
                                    type="file"
                                    accept=".xlsx, .xls"
                                    onChange={async (e) => {
                                        ...
                                        try {
                                            const res = await fetch("/api/spip/instalasi/import", {
                                                method: "POST",
                                                body: formData
                                            });
                ...
                <Table>
                    <TableHeader className="bg-gray-50/50">
                        <TableRow>
                            ...
                            <TableHead className="font-bold uppercase text-[10px]">Kategori</TableHead>
                    ...
                    <TableBody>
                        {isLoading ? (
                            ...
                            <TableRow>
                                <TableCell colSpan={7} className="h-40 text-center text-muted-foreground italic">Data tidak ditemukan.</TableCell>
                            </TableRow>
                        ) : (
                            items.map((item: any, index: number) => {
                                ...
                                return (
                                    <TableRow key={item.id} className="hover:bg-gray-50/50">
                                        ...
                                        <TableCell>
                                            <Badge variant="outline" className="text-[9px] font-bold bg-gray-50">
                                                {item.kategori}
                                            </Badge>
                                        </TableCell>
                                        ...
                                        <TableCell className="text-right">
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end" className="w-32">
                                                    <DropdownMenuItem onClick={() => navigate(`/workspace/hse/ko/spip/instalasi/${item.id}`)}>
