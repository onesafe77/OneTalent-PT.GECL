...
import { Download, Search, X, ChevronLeft, ChevronRight, ExternalLink } from "lucide-react";
import { angka, BULAN } from "@/lib/hse-statistik";

/**
 * Database Violation Safe Distance — satu baris per kejadian, seluruh kolom sheet.
 * Halaman penelusuran; analisisnya ada di Dashboard Jarak Aman.
 */

const KOSONG = <span className="text-muted-foreground">—</span>;

const KOSONG_SARINGAN = {
    tahun: "all", bulan: "all", jenis: "all", status: "all",
    sanksi: "all", shift: "all", jalur: "all", statusSanksi: "all", rentang: "all",
};
...
export default function SdDatabase() {
    ...
    const [f, setF] = useState<Record<string, string>>({ ...KOSONG_SARINGAN });

    const q = new URLSearchParams({ ...f, cari, hal: String(hal), per }).toString();
    const { data, isLoading } = useQuery<any>({ queryKey: [`/api/hse/sd-violations?${q}`] });
    ...
    const reset = () => { setF({ ...KOSONG_SARINGAN }); setCari(""); setHal(1); };
    ...
    const SARING = [
        ...
        { k: "rentang", label: "Semua jarak", opsi: p.rentang ?? [] },
        { k: "status", label: "Semua status", opsi: p.status ?? [], ekstra: [{ v: "belum", n: "Belum selesai" }] },
        { k: "statusSanksi", label: "Semua status sanksi", opsi: p.statusSanksi ?? [] },
    ...
    return (
        <div className="space-y-6 p-6 md:p-8">
            <div className="flex flex-col items-start justify-between gap-4 md:flex-row md:items-end">
                <div>
                    <h1 className="text-[28px] font-semibold tracking-[-0.03em] text-foreground">Database Violation Safe Distance</h1>
                ...
                <Button variant="outline" asChild>
                    <a href={`/api/hse/sd-violations/export?${new URLSearchParams({ ...f, cari }).toString()}`}>
            ...
            <Card className="rounded-xl border border-border bg-card">
                <CardContent className="space-y-3 p-4">
                    <div className="relative">
                        ...
                        <Input className="h-9 pl-9" placeholder="Cari NIK, nama, unit, jabatan, sanksi, kode, kategori, jalur, lokasi…"
                            value={cari} onChange={(e) => { setCari(e.target.value); setHal(1); }} />
                    </div>
                    <div className="flex flex-wrap items-center gap-2">
                        {SARING.map((s) => (
                            <Select key={s.k} value={f[s.k]} onValueChange={(v) => set(s.k, v)}>
                                <SelectTrigger className="h-9 w-[152px]"><SelectValue /></SelectTrigger>
                                <SelectContent>
                                    <SelectItem value="all">{s.label}</SelectItem>
                                    {((s as any).ekstra ?? []).map((e: any) => <SelectItem key={e.v} value={e.v}>{e.n}</SelectItem>)}
                                    {s.opsi.map((o: string) => <SelectItem key={o} value={o}>{(s as any).nama ? (s as any).nama(o) : o}</SelectItem>)}
                                </SelectContent>
                            </Select>
                        ))}
                        {aktif > 0 && (
                        ...
                        <Table>
                            <TableHeader>
                                <TableRow>
                                    ...
                                    <TableHead>Jabatan</TableHead><TableHead>Pelanggaran</TableHead>
                                    <TableHead className="text-right">Jarak aman</TableHead>
                                    <TableHead>Lokasi</TableHead><TableHead>Jalur</TableHead><TableHead>Shift</TableHead>
                                    <TableHead>Sanksi</TableHead><TableHead>Berlaku s.d.</TableHead>
                                    <TableHead>Status</TableHead><TableHead className="text-right">Durasi</TableHead>
                                    <TableHead className="pr-4">Bukti</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {isLoading ? (
                                    <TableRow><TableCell colSpan={16} className="py-10 text-center text-muted-foreground">Memuat…</TableCell></TableRow>
                                ) : baris.length === 0 ? (
                                    <TableRow><TableCell colSpan={16} className="py-12 text-center text-muted-foreground">
                                    ...
                                    <TableRow key={x.kunci}>
                                        ...
                                        <TableCell className="whitespace-nowrap font-medium text-foreground">{x.unit || KOSONG}</TableCell>
                                        ...
                                        <TableCell className="max-w-[190px] truncate text-[12px] text-muted-foreground" title={x.jabatan}>
                                            {x.jabatan || KOSONG}
                                        </TableCell>
                                        ...
                                        {/* Makin kecil detiknya makin rawan — di bawah 2 detik ditandai. */}
                                        <TableCell className="whitespace-nowrap text-right tabular-nums">
                                            {x.detik !== null ? (
                                                <span className={x.detik < 2 ? "font-semibold text-red-600 dark:text-red-400" : ""}>
                                                    {x.detik.toFixed(2)} dtk
                                                </span>
                                            ) : KOSONG}
                                        </TableCell>
                                        <TableCell className="whitespace-nowrap">{x.lokasiKm ? `KM ${x.lokasiKm}` : KOSONG}</TableCell>
                                        <TableCell>{x.jalur || KOSONG}</TableCell>
                                        <TableCell className="whitespace-nowrap text-[12px]">{x.shift || KOSONG}</TableCell>
                                        <TableCell className="whitespace-nowrap">
                                            {x.sanksi ? (
                                        ...
                                        <TableCell className="whitespace-nowrap tabular-nums">
                                            {tgl(x.masaBerlakuSanksi) ?? KOSONG}
                                            {x.statusSanksi === "Aktif" && (
                                                <Badge className="ml-1.5 border-amber-200 bg-amber-50 text-[10px] text-amber-700 dark:border-amber-900/50 dark:bg-amber-950 dark:text-amber-400">Aktif</Badge>
                                            )}
                                        </TableCell>
                                        <TableCell className="whitespace-nowrap">
                                            {x.status === "Closed" ? (
                                        ...
                                        {/* Durasi negatif = galat sumber, ditampilkan apa adanya agar bisa dibetulkan. */}
                                        <TableCell className="whitespace-nowrap text-right tabular-nums">
                                            {x.durasiClose !== null ? (
                                                <span className={x.durasiClose < 0 ? "text-red-600 dark:text-red-400" : ""}
                                                    title={x.durasiClose < 0 ? "Tanggal pemenuhan mendahului tanggal kejadian — galat data sumber" : undefined}>
                                                    {angka(x.durasiClose)} hari
                                                </span>
                                            ) : KOSONG}
                                        </TableCell>
                                        <TableCell className="pr-4">
                                            <div className="flex items-center gap-2">
                                                {x.capture && <a href={x.capture} target="_blank" rel="noreferrer" className="text-muted-foreground hover:text-foreground" title="Foto pelanggaran"><ExternalLink className="h-3.5 w-3.5" /></a>}
                                                {x.evidence && <a href={x.evidence} target="_blank" rel="noreferrer" className="text-[12px] underline decoration-border underline-offset-2 hover:text-foreground">Bukti</a>}
                                                {!x.capture && !x.evidence && KOSONG}
                                            </div>
                                        </TableCell>
                                    </TableRow>
                                ))}
