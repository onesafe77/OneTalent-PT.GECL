...
interface SessionWithDetails extends SidakImpactSession {
    records?: SidakImpactRecord[];
    observers?: SidakImpactObserver[];
}

export default function SidakImpactHistory() {
    ...
    const { data: sessions, isLoading } = useQuery<SessionWithDetails[]>({
        queryKey: ['/api/sidak-impact/sessions'],
    });

    const uploadPhotosMutation = useMutation({
        mutationFn: async ({ sessionId, base64Photos }: { sessionId: string; base64Photos: string[] }) => {
            const res = await apiRequest(`/api/sidak-impact/${sessionId}/photos`, "POST", {
                photos: base64Photos
            });
            return res;
        },
    ...
    const deletePhotoMutation = useMutation({
        mutationFn: async ({ sessionId, photoIndex }: { sessionId: string; photoIndex: number }) => {
            const res = await apiRequest(`/api/sidak-impact/${sessionId}/photos/${photoIndex}`, "DELETE");
            return res;
        },
        onSuccess: (data) => {
            queryClient.invalidateQueries({ queryKey: ['/api/sidak-impact/sessions'] });
    ...
    const handleDownloadPDF = async (sessionId: string) => {
        try {
            ...
            const response = await fetch(`/api/sidak-impact/${sessionId}`);
            if (!response.ok) throw new Error('Gagal mengambil data session');
            const data = await response.json();

            const { generateSidakImpactPDF } = await import('@/lib/sidak-impact-pdf-utils');
            const doc = await generateSidakImpactPDF({
                session: data.session,
                records: data.records,
                observers: data.observers
            });

            const fileName = `Sidak_Impact_${data.session.tanggal}_${data.session.id.substring(0, 5)}.pdf`;
            doc.save(fileName);
    ...
    const handleDownloadJPG = async (sessionId: string) => {
        try {
            ...
            const { downloadSidakImpactAsJpg } = await import('@/lib/sidak-impact-pdf-utils');
            const fileName = `Sidak_Impact_${data.session.tanggal}_${Date.now()}.jpg`;
            await downloadSidakImpactAsJpg({
                session: data.session,
                records: data.records,
                observers: data.observers
            }, fileName);
                ...
                {isLoading ? (
                    ...
                    <Card className="text-center py-12 border-dashed border-2 border-gray-200">
                        <CardContent>
                            ...
                            <p className="text-gray-500 font-medium">Belum ada riwayat inspeksi impact</p>
                            <EmptyStateCreateButton href="/workspace/sidak/impact/new" label="Mulai Inspeksi Baru" className="mt-6 bg-blue-600 hover:bg-blue-700" />
                        </CardContent>
                    </Card>
                ) : (
                            ...
                            <Card key={session.id} className="overflow-hidden border-none shadow-sm rounded-2xl bg-white dark:bg-gray-800 ring-1 ring-gray-100">
                                ...
                                <CardContent className="p-5">
                                    <div className="flex justify-between items-start mb-4">
                                        ...
                                        <Badge className="bg-blue-50 text-blue-700 border-none rounded-full px-3 py-1">
                                            {session.totalImpact} Unit
                                        </Badge>
                                    </div>
