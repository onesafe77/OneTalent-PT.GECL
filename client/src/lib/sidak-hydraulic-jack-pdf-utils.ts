...
// Inspection items matching the official GECL Hydraulic Jack template
const INSPECTION_ITEMS = [
    ...
    { id: '3', label: '3. Apakah tidak terdapat rembesan oli hydraulic pada body jack?' },
    { id: '4', label: '4. Apakah kondisi pompa hydraulic jack berfungsi dengan baik?' },
    { id: '5', label: '5. Apakah valve pengunci hydraulic masih berfungsi dengan baik?' },
...
export async function generateSidakHydraulicJackPDF(data: HydraulicJackPDFData): Promise<jsPDF> {
    ...
    const drawHeader = (yStart: number): number => {
        ...
        pdf.setFontSize(11);
        pdf.setTextColor(0, 0, 0);
        pdf.text('CHECKLIST INSPEKSI PERALATAN HYDRAULIC JACK', pageWidth / 2, yPosition + 4, { align: 'center' });
        pdf.setFont('helvetica', 'italic');
        pdf.setFontSize(7);
        pdf.text('Formulir ini digunakan untuk pencatatan hasil inspeksi hydraulic jack', pageWidth / 2, yPosition + 8, { align: 'center' });
    ...
    // Build the table data for all records (each Hydraulic Jack unit)
        ...
        data.records.forEach((record, recordIndex) => {
            ...
            tableRows.push({
                ...
                data: [
                    ...
                    { content: `HYDRAULIC JACK – No Register Peralatan : ${registerNo}`, colSpan: 4, styles: { fontStyle: 'bold', fillColor: [200, 200, 200], halign: 'left' } }
                ]
            });

            // Flat inspection items (no categories)
...
export async function downloadSidakHydraulicJackAsJpg(data: HydraulicJackPDFData, filename: string): Promise<void> {
    ...
    try {
        ...
        const pdf = await generateSidakHydraulicJackPDF(data);
