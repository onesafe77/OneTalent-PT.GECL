...
// ============================================
// SIDAK IMPACT INSPECTION PDF GENERATOR
// Document Code: BIB – HSE – ES – F – 3.02 – 88
...
// Inspection items matching the official GECL Impact template
const INSPECTION_ITEMS = [
    ...
    { id: "3", label: "3. Apakah tidak ada keretakan pada bagian Impact? (visual check)" },
    { id: "4", label: "4. Apakah tidak ada kerusakan pada bagian adaptor angin?" },
    { id: "5", label: "5. Apakah valve switch putaran kanan dan kiri berfungsi dengan baik?" },
    { id: "6", label: "6. Apakah selang supply untuk angin tidak ada kebocoran dan kerusakan?" },
    { id: "7", label: "7. Apakah setelan torsi berfungsi dengan baik?" }
];
...
export async function generateSidakImpactPDF(data: ImpactPDFData): Promise<jsPDF> {
    ...
    const drawHeader = (yStart: number): number => {
        ...
        pdf.text('BIB – HSE – ES – F – 3.02 – 88', pageWidth - margin, yPosition + 5, { align: 'right' });
        ...
        pdf.setFontSize(11);
        pdf.setTextColor(0, 0, 0);
        pdf.text('CHECKLIST INSPEKSI IMPACT', pageWidth / 2, yPosition + 4, { align: 'center' });
        pdf.setFont('helvetica', 'italic');
        pdf.setFontSize(7);
        pdf.text('Formulir ini digunakan untuk pencatatan hasil inspeksi impact system', pageWidth / 2, yPosition + 8, { align: 'center' });
        ...
        data.records.forEach((record, recordIndex) => {
            ...
            tableRows.push({
                ...
                data: [
                    ...
                    { content: `IMPACT – No Register Peralatan : ${registerNo}`, colSpan: 4, styles: { fontStyle: 'bold', fillColor: [200, 200, 200], halign: 'left' } }
                ]
            });
...
export async function downloadSidakImpactAsJpg(data: ImpactPDFData, filename: string): Promise<void> {
    ...
    try {
        ...
        const pdf = await generateSidakImpactPDF(data);
